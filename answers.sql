...
SELECT email FROM customers LEFT JOIN orders on customers.id = orders.customer_id GROUP BY email HAVING SUM(num_cupcakes) > 100 ORDER BY REVERSE(email);
...
SELECT fname, lname, email FROM customers INNER JOIN orders ON customers.id = orders.customer_id WHERE orders.cupcake_id = (SELECT id FROM cupcakes WHERE cupcakes.name = 'funfetti') AND orders.processed = 'True' GROUP BY fname, lname, email;